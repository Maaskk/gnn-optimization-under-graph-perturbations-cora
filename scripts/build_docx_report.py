...
def add_cover(doc: Document) -> None:
    ...
    add_para(
        doc,
        "Projet 13 - Option 4: Robustesse des GNN face au bruit dans le graphe",
        size=12,
        color=MUTED,
        align=WD_ALIGN_PARAGRAPH.CENTER,
        after=42,
    ...
    add_data_table(
        ...
        ["Field", "Value"],
        rows,
        widths=[Inches(1.55), Inches(4.75)],
        header_fill="11161F",
        font_size=9.2,
    )
    doc.add_page_break()
...
def build_report() -> Document:
    ...
    add_data_table(
        ...
        key_rows,
        widths=[Inches(1.8), Inches(1.55), Inches(2.95)],
        header_fill="D71920",
        font_size=9,
    )
    ...
    add_para(
        doc,
        "Kipf and Welling introduced the two-layer GCN model for semi-supervised classification on citation networks and reported strong Cora performance with a small labeled training set [1]. GraphSAGE and GAT later expanded the GNN family through inductive aggregation and attention mechanisms [2, 3].",
    )
    add_para(
        doc,
    ...
    add_data_table(
        ...
        ["Property", "Value", "Note"],
        dataset_rows,
        widths=[Inches(1.65), Inches(1.25), Inches(3.4)],
        header_fill="11161F",
        font_size=9,
    )
    ...
    add_data_table(
        ...
        protocol_rows,
        widths=[Inches(1.65), Inches(1.55), Inches(3.1)],
        header_fill="11161F",
        font_size=9,
    )
    ...
    add_callout(
        doc,
        "Interpretation",
        "The 70.2% aggregate accuracy is lower because it averages clean and perturbed graphs. It should not be compared directly with a clean-only Cora benchmark.",
    )
    ...
    for row in clean:
        clean_rows.append(
            [
                ...
                fmt(row["macro_f1"]),
                fmt(row["final_loss"]),
                f"{float(row['training_time_seconds']):.1f}s",
            ]
        )
    add_data_table(
        doc,
        ["Optimizer", "Accuracy", "Macro F1", "Final loss", "Time"],
        clean_rows,
        widths=[Inches(1.35), Inches(1.1), Inches(1.1), Inches(1.25), Inches(1.2)],
        font_size=9,
    )
    add_figure(
        doc,
        FIGURES / "clean_accuracy_macro_f1.png",
        "Figure 1 - Clean Cora test accuracy and macro F1-score.",
        width=5.9,
    )
    add_para(
        doc,
        "Adam and AdamW tie at 0.799 clean test accuracy. RMSProp follows very closely at 0.795. AdaGrad learns but remains lower, while SGD stays near random-guess behavior under the fixed learning rate.",
    )

    add_heading(doc, "5.3 Aggregate performance", 2)
    add_figure(
        doc,
        ASSETS / "aggregate_accuracy_f1.png",
        "Figure 2 - Aggregate mean accuracy and macro F1 across all 65 runs.",
        width=6.2,
    )
    pivot = {row["optimizer"]: {} for row in summary}
    for row in summary:
    ...
    add_figure(
        doc,
        FIGURES / "edge_removal_accuracy.png",
        "Figure 4 - Edge removal is tolerated best by adaptive optimizers.",
        width=5.9,
    )
    add_para(
        doc,
        "Edge removal produces the smallest drop. Adam, AdamW, and RMSProp remain close to their clean scores even at 30% edge deletion, showing that the Cora graph retains enough redundant neighborhood information within this severity range.",
    )
    add_figure(
        doc,
        FIGURES / "fake_edge_accuracy.png",
        "Figure 5 - Fake edge addition causes a monotonic structural degradation.",
        width=5.9,
    )
    add_para(
        doc,
        "Fake edges are more damaging than edge removal because they add misleading neighborhoods. Adam has the smallest degradation among the adaptive optimizers, while AdaGrad falls more sharply under injected links.",
    )

    add_heading(doc, "5.5 Training dynamics", 2)
    add_figure(
        doc,
        FIGURES / "clean_loss_convergence.png",
        "Figure 6 - Training and validation loss over 200 epochs on the clean graph.",
        width=6.2,
    )
    add_para(
        doc,
        "AdamW reaches the lowest training loss, but the wider train/validation gap indicates tighter fitting to the small 140-node training set. RMSProp reaches a strong validation plateau quickly, while SGD does not escape its near-random initial behavior.",
    )

    add_heading(doc, "5.6 Accuracy drop matrix", 2)
    add_figure(
        doc,
        ASSETS / "accuracy_drop_heatmap.png",
        "Figure 7 - Accuracy drop from each optimizer's own clean baseline.",
        width=6.2,
    )
    add_para(
        doc,
    ...
    limitation_rows = [
        [
            "Single seed",
            "All experiments use seed 42; multiple seeds would give confidence intervals.",
        ],
        [
            "Fixed learning rate",
            "The protocol is fair for comparison but may disadvantage SGD, which often needs tuning or momentum.",
        ],
        [
            "One dataset",
            "Cora is standard but small; PubMed and CiteSeer would test scale and transfer.",
        ],
        [
            "One architecture",
            "The conclusions apply to this 2-layer GCN, not necessarily GAT or GraphSAGE.",
        ],
    ...
    add_data_table(
        ...
        ["Limitation", "Meaning"],
        limitation_rows,
        widths=[Inches(1.65), Inches(4.65)],
        header_fill="11161F",
        font_size=9,
    )
    add_para(
        doc,
        "These limitations do not invalidate the project. They define the interpretation: this is a controlled optimizer comparison under graph noise, not a universal claim about every GNN or every citation dataset.",
    )
    ...
    add_callout(
        ...
        "Use Adam as the default optimizer for the final project demonstration, and explain that its advantage is not only clean accuracy but also stable performance across feature and structural perturbations.",
    )
    ...
    annex_rows = [
        ...
        [
            ...
            "https://maaskk.github.io/gnn-optimization-under-graph-perturbations-cora/",
        ],
